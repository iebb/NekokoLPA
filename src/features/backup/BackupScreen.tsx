...
export default function Backup(_props: RootScreenProps<'Backup'>) {
  ...
  return (
    <Screen title={t('main:backup')} subtitle={t('main:backup_description')}>
      <YStack gap={20} flex={1}>
        <XStack gap={12}>
          <TButton flex={1} backgroundColor="$btnBackground" onPress={exportFile} borderRadius={12}>
            <XStack alignItems="center" gap={10}>
              <Download size={20} color="$color0" />
              <TText color="$color0">{t('main:backup_export')}</TText>
            </XStack>
          </TButton>
          <TButton flex={1} backgroundColor="$btnBackground" onPress={importData} borderRadius={12}>
            <XStack alignItems="center" gap={10}>
              <Upload size={20} color="$color0" />
              <TText color="$color0">{t('main:backup_import')}</TText>
            </XStack>
          </TButton>
        </XStack>
      </YStack>
