...
export function parseMetadata(metadata: ProfileMetadataMap, t: TFunction, parseCountry = true) {
  const tags: Tag[] = [];
  let nickname = (metadata.profileNickname || metadata.profileName || metadata.serviceProviderName || '').trim();

  if (nickname) {
    // Remove order suffix if present
    nickname = nickname.replace(/\s?\^[a-z]{3}$/i, '').trim();
    ...
    for (const eDate of eDates) {
      if (eDate && eDate.length > 1) {
        ...
        const colorSet = days < 0 ? TAG_COLORS.red : (days < 7 ? TAG_COLORS.orange : TAG_COLORS.green);
        tags.push({
          tag: 'date',
          value: t(days < 0 ? 'main:profile_date_past' : 'main:profile_date_remaining', { date: date8, remaining: Math.abs(days) }),
          rawValue: `d:${date6}`,
          color: colorSet.fg,
    ...
    for (const match of it) {
      tags.push({ tag: 'text', value: match[1].replaceAll("_", " "), rawValue: `t:${match[1]}`, color: TAG_COLORS.blue.fg, backgroundColor: TAG_COLORS.blue.bg });
  ...
  try {
    if (mccMncInfo.MCC) {
      const lastValue = JSON.parse(countryList.getString(mccMncInfo.MCC) || '{}');
  ...
  if (countryMatch) {
    ...
    if (parseCountry) nickname = nickname.replace(countryMatch[0], "").trim();
  }

  return { name: nickname, tags, country: countryEmoji, mccMnc: mccMncInfo };
}
...
export function parseMetadataOnly(metadata: ProfileMetadataMap) {
  ...
  if (nickname) {
    nickname = nickname.replace(/\s?\^[a-z]{3}$/i, '').trim();
    const eDates = nickname.matchAll(/\s*d:(2[012])?(\d\d)(\d\d)(\d\d)/g);
    for (const eDate of eDates) if (eDate && eDate.length > 1) nickname = nickname.replace(eDate[0], '').trim();

    const it = nickname.matchAll(/\s*t:(\S+)/g);
    for (const match of it) {
      tags.push({ tag: 'text', value: match[1].replaceAll("_", " "), rawValue: `t:${match[1]}` });
      nickname = nickname.replace(match[0], '').trim();
    }
  }
