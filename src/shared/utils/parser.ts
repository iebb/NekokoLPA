...
export function parseMetadata(metadata: ProfileMetadataMap, t: TFunction, parseCountry = true) {
  ...
  if (nickname) {
    ...
    for (const eDate of eDates) {
      if (eDate && eDate.length > 1) {
        ...
        tags.push({
          tag: 'date',
          value: t(days < 0 ? 'main:profile_date_past' : 'main:profile_date_remaining', {
            date: date8,
  ...
  const mccMncInfo = resolveMccMnc(metadata.profileOwnerMccMnc);
  try {
    if (mccMncInfo.MCC) {
      const lastValue = JSON.parse(countryList.getString(mccMncInfo.MCC) || '{}');
      lastValue[metadata.profileOwnerMccMnc] =
        mccMncInfo.Operator || metadata.serviceProviderName || metadata.profileOwnerMccMnc;
  ...
  const countryMatch = nickname.match(/[🇦-🇿]{2}/u);
  if (countryMatch) {
...
export function parseMetadataOnly(metadata: ProfileMetadataMap) {
  ...
  if (nickname) {
    ...
    const eDates = nickname.matchAll(/\s*d:(2[012])?(\d\d)(\d\d)(\d\d)/g);
    for (const eDate of eDates)
      if (eDate && eDate.length > 1) nickname = nickname.replace(eDate[0], '').trim();
